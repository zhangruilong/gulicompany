...
Ext.define('Ext.ux.DateTimePicker', {
    ...
    renderTpl: [
        '<div id="{id}-innerEl" data-ref="innerEl" role="presentation">',
            ...
            '<tpl if="showToday">',
              ...
              //也就是增加 时、分、秒、OK控件
			  '<div id="{id}-footerEl" role="presentation" style="background-color:#D9E5F3;border-top:0px solid #99BCE8;">{%this.renderHour(values, out)%}{%this.renderMinute(values, out)%}{%this.renderSecond(values, out)%}<center>{%this.renderOkQueDingBtn(values, out)%}&nbsp;&nbsp;{%this.renderTodayBtn(values, out)%}</center></div>',
            '</tpl>',
    ...
    beforeRender: function() {
		...
		//在beforeRender事件中，添加时、分、秒、OK控件的事件
   ...
   /** 
     ...
     * @inheritdoc
     */
    beforeDestroy: function() {	//在beforeDestroy释放事件中添加释放OK按钮的事件
        var me = this;
