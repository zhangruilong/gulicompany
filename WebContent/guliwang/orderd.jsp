...
function initOrderm(data){
 	 $.each(data.root, function(i, item) {
 		...
 		$(".order-detail-user").append('<p>收货人:'+item.ordermconnect+item.ordermphone+'</p>'+
...
function initOrderd(data){
	...
	$(".order-detail-wrapper").append('<ul>');
	 $.each(data.root, function(i, item) {
		$(".order-detail-wrapper").append('<li><span class="fl">'+item.orderdname+
		    	'</span><span class="fl">¥'+item.orderdprice+'/'+item.orderdunit+
		    	'</span><span class="fl">x'+item.orderdnum+'</span><span class="fr"> '+
		    	item.orderdmoney+'元</span></li>');
    });
	$(".order-detail-wrapper").append('</ul>');
